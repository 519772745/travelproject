...
function star_on(obj){
	$(obj).attr("src","detail/img/star1.png");
}
function star_off(obj){
	$(obj).attr("src","detail/img/star.png");
}
/*鼠标划入划出点赞链接时调用的js
...
function praise_on(obj){
	$(obj).children().attr("src","detail/img/like2.png");
}
function praise_off(obj){
	$(obj).children().attr("src","detail/img/like.png");
}
/*鼠标划入划出举报链接时调用的js
...
function report_on(obj){
	$(obj).children().attr("src","detail/img/report3.png");
}
function report_off(obj){
	$(obj).children().attr("src","detail/img/report.png");
}
	...
	$.ajax({
		...
		success(data){	
				...
				if(data.status=="collected"){ 
					...
					$(obj).append('<img src="detail/img/star1.png" id="star" width="16" height="16" title="已收藏">'); //添加图片
				}
				/*若用户现未收藏*/
				if(data.status=="uncollected"){
					$(obj).children().remove(); //删除收藏链接下的子元素（图片及收藏字样）
					$(obj).text("收藏("+data.collectNum+")" ); //添加收藏字样
					$(obj).append('<img src="detail/img/star.png" id="star" onmouseover="star_on(this)" onmouseout="star_off(this)" width="16" height="16" title="未收藏">');//添加图片
				}
			}	
	});
}
 ...
 function praise_col(travelNoteId,obj){
		$.ajax({
			...
			success(data){	
					...
					$(obj).text(""); //清空点赞连接下的值
					$(obj).append("<img src='detail/img/like2.png' id='like' width='16' height='16'>"); //添加点赞连接下的图片
...
function report_col(travelNoteId,obj){
	$.ajax({
		...
		success(data){	
				...
				$(obj).children().attr("src","detail/img/report3.png"); //更改举报的图标
				alert("举报成功"); //提示已举报成功
			}	
	});
}
