...
<div class="container" style="margin-top: 130px;">
	<div class="row">
		...
		<!--设置隐藏域 这是再编辑时接收数据的隐藏域 通过js传到编辑框里-->
								<textarea rows="10" cols="10" id="testcon" style="display:none;">
							        ${reEditor.content }
								</textarea>
				<!-- 上传图片 -->
				<div>
		...
		<!--接收父级的tag-->
							<c:forEach items="${reEditor.tagTravelNote}" var="tagi">
								<c:if test="${tagi.tags.parentTag==null }">
									<input id="tagPName" type="hidden" value="${tagi.tags.tagName }" > 
								</c:if>
							</c:forEach>						
		...
		<!--接收自定义的tag-->
							<div style="display:none;">
								<c:forEach items="${reEditor.tagTravelNote}" var="tagi">
									<c:if test="${tagi.tags.parentTag!=null }">
										<p class="tags">${tagi.tags.tagName},</p>
									</c:if>
								</c:forEach>
							</div>							
							<!--设置隐藏域——标签的值-->
	 	...
	 	<!--接收权限的数据-->
							<input id="statusValue" type="hidden" value="${reEditor.status }" > 
						...
						<div class="si">
							...
							<input id="getTopicId" type="hidden" value="${reEditor.topic }" > 							    
						</div>
