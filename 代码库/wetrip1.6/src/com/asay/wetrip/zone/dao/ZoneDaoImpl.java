...
public class ZoneDaoImpl {
	...
	public List<TravelNote> findTravelByUsername(int pageNum, int pageSize,String username,String date){
		...
		System.out.println(date);
		 ...
		 for(TravelNote t : (List<TravelNote>)q.list()) {
		 ...
		 if((pageSize*(pageNum-1))+pageSize<=list.size())
		 {
			 ...
			 travelList=list.subList(pageSize*(pageNum-1), (pageSize*(pageNum-1))+pageSize);
		 }
		 else {
			 travelList=list.subList(pageSize*(pageNum-1),list.size());
		 }
	...
	@SuppressWarnings({ "rawtypes"})
	public List<TravelNote> findTravelByLoginUsername(int pageNum, int pageSize,String loginUsername,String date){
		Session session=this.sessionFactory.getCurrentSession();
		...
		UserDetail userDetail = (UserDetail) q2.uniqueResult();
		q.setParameter("userDetail",userDetail);					
		List<TravelNote> tl=(List<TravelNote>)q.list();
		 List<TravelNote> list= new ArrayList<TravelNote>();
		 for(int i=0;i<tl.size();i++) {
			 TravelNote t=tl.get(i);
			 Date publishDate=t.getPublishtime();
